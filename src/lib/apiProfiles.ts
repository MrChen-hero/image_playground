...
export function normalizeReferenceCompressionTarget(value: unknown, fallback = 650): number {
  const numeric = typeof value === 'number' ? value : Number(value)
...
export function normalizeSettings(input: Partial<AppSettings> | unknown): AppSettings {
  ...
  return {
    ...
    referenceCompressionEnabled: typeof record.referenceCompressionEnabled === 'boolean' ? record.referenceCompressionEnabled : false,
    referenceCompressionTargetKb: normalizeReferenceCompressionTarget(record.referenceCompressionTargetKb),
