...
type ProcessedInputImage = InputImage & {
  preprocessResult: ReferenceImagePreprocessResult
}
...
export async function editOutputs(task: TaskRecord) {
  ...
  for (const imgId of task.outputImages) {
    ...
    if (dataUrl) {
      ...
      const id = await storeImage(processed.dataUrl, 'upload')
      if (existingInputIds.has(id)) continue

      cacheImage(id, processed.dataUrl)
...
export async function addImageFromFile(file: File): Promise<AddReferenceImageResult | undefined> {
  ...
  const added = !useStore.getState().inputImages.some((img) => img.id === image.id)
  useStore.getState().addInputImage({ id: image.id, dataUrl: image.dataUrl })
...
export async function createInputImageFromFile(file: File): Promise<ProcessedInputImage | null> {
...
async function createInputImageFromDataUrl(dataUrl: string): Promise<ProcessedInputImage> {
  const processed = await preprocessReferenceImageDataUrl(dataUrl, useStore.getState().settings)
  const id = await storeImage(processed.dataUrl, 'upload')
  cacheImage(id, processed.dataUrl)
  return {
    id,
