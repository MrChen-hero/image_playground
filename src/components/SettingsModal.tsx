...
export default function SettingsModal() {
  ...
  useEffect(() => {
    ...
    setReferenceCompressionTargetInput(String(nextDraft.referenceCompressionTargetKb))
  ...
  const handleClose = () => {
    ...
    const normalizedReferenceCompressionTarget = referenceCompressionTargetInput.trim() === ''
      ? DEFAULT_SETTINGS.referenceCompressionTargetKb
      : normalizeReferenceCompressionTarget(referenceCompressionTargetInput, draft.referenceCompressionTargetKb)
    const nextDraft = {
      ...draft,
      agentMaxToolRounds: normalizedAgentMaxToolRounds,
      referenceCompressionTargetKb: normalizedReferenceCompressionTarget,
    ...
    setReferenceCompressionTargetInput(String(normalizedReferenceCompressionTarget))
    commitSettings(nextDraft)
  ...
  const commitReferenceCompressionTarget = useCallback(() => {
    const value = referenceCompressionTargetInput.trim() === ''
      ? DEFAULT_SETTINGS.referenceCompressionTargetKb
      : normalizeReferenceCompressionTarget(referenceCompressionTargetInput, draft.referenceCompressionTargetKb)
    setReferenceCompressionTargetInput(String(value))
    if (value !== draft.referenceCompressionTargetKb) commitSettings({ ...draft, referenceCompressionTargetKb: value })
  }, [referenceCompressionTargetInput, draft])
          ...
          <div className="flex-1 flex flex-col min-w-0 min-h-0 bg-transparent relative overflow-hidden">
            ...
            {activeTab === 'general' && (
              <div className="space-y-4">
                ...
                <div className="block">
                  <div className="mb-1 flex items-center justify-between">
                    <span className="block text-sm text-gray-600 dark:text-gray-300">参考图自动压缩</span>
                    <button
                      type="button"
                      onClick={() => commitSettings({ ...draft, referenceCompressionEnabled: !draft.referenceCompressionEnabled })}
                      className={`relative inline-flex h-4 w-7 items-center rounded-full transition-colors ${draft.referenceCompressionEnabled ? 'bg-blue-500' : 'bg-gray-300 dark:bg-gray-600'}`}
                      role="switch"
                      aria-checked={draft.referenceCompressionEnabled}
                      aria-label="参考图自动压缩"
                    >
                      <span className={`inline-block h-3 w-3 transform rounded-full bg-white shadow transition-transform ${draft.referenceCompressionEnabled ? 'translate-x-[14px]' : 'translate-x-[2px]'}`} />
                    </button>
                  </div>
                  <div data-selectable-text className="text-xs text-gray-500 dark:text-gray-500">
                    开启后，上传、粘贴、拖拽、替换参考图和编辑输出加入参考图时，会先在浏览器端压缩。
                  </div>
                </div>
                <label className={`block ${draft.referenceCompressionEnabled ? '' : 'opacity-60'}`}>
                  <span className="mb-1.5 block text-sm text-gray-600 dark:text-gray-300">参考图压缩目标 (KB)</span>
                  <input
